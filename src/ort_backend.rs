...
impl OrtBackend {
    ...
    pub fn run(&self, xs: Array<f32, IxDyn>) -> Result<Vec<Array<f32, IxDyn>>> {
        // ORT inference
        match self.dtype() {
            TensorElementDataType::Float16 => self.run_fp16(xs),
            TensorElementDataType::Float32 => self.run_fp32(xs),
            _ => todo!(),
        }
    }

    pub fn run_fp16(&self, xs: Array<f32, IxDyn>) -> Result<Vec<Array<f32, IxDyn>>> {
        // f32->f16
    ...
    pub fn run_fp32(&self, xs: Array<f32, IxDyn>) -> Result<Vec<Array<f32, IxDyn>>> {
        // h2d
        let xs = CowArray::from(xs);
        let xs = vec![Value::from_array(self.session.allocator(), &xs)?];
        ...
        Ok(ys
            .iter()
            .map(|x| x.try_extract::<_>().unwrap().view().clone().into_owned())
            .collect::<Vec<Array<_, _>>>())
    }
